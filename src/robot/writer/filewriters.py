...
class ColumnAligner(ast.NodeVisitor):
    ...
    def __init__(self, widths):
        ...
        self.indent = 0
        self._first_statement_seen = False
    ...
    def visit_Statement(self, statement):
        ...
        for line in statement.lines:
            ...
            for token, width in zip(line, widths):
                exp_pos += width
                # TODO: maybe extract method to resolve if first keyword call
                # goes on the same line as the test/uk name
                if line_pos == 0 and not self._first_statement_seen and \
                        self._test_name_len < 18:
                    exp_pos -= self._test_name_len
                    self._first_statement_seen = True
...
class Writer(ast.NodeVisitor):
    ...
    def visit_TestOrKeyword(self, node):
        if self._test_or_kw_seen:
            self._write_newline()
        # TODO: put want_content_on_first_row or similar to context
    ...
    def _write(self, content):
        self.configuration.output.write(content)
